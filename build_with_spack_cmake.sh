...
BRANCH=cmake
BUILD_SPEC="ssparse@${BRANCH} target=${SPACK_TARGET}"
...
spack spec ${BUILD_SPEC} > /tmp/ssparse_spec_${BRANCH}

# Extracts the exact spec of a dependency then returns the install directory
function location() {
    local spec=$(grep "\^${1}@" /tmp/ssparse_spec_${BRANCH} | sed 's/\s.*^//g')
...
add_to_install_rpath $(location libex)/lib
...
add_to_prefix_path $(location libex)
